...
jq '.differences | .[] | select(.dimension.metric == "instructions") |  select(.diff >= 100000000) | [.dimension.benchmark, .diff, (.reldiff * 100)]' temp.json > temp1
jq '.differences | .[] | select(.dimension.metric == "instructions") |  select(.diff <= -100000000) | [.dimension.benchmark, .diff, (.reldiff * 100)]' temp.json > temp2
